...
import { buildFallbackOverview, buildFallbackTokenContext, fallbackWorkspaceTokens } from "@/lib/fallback-workspace";
...
export default async function AppPage({
  ...
  const liveBackendUnavailable = liveTokens.length === 0;
  ...
  const resolvedContext = context ?? buildFallbackTokenContext(selectedSymbolCandidate, tokens);
  ...
  const overview = activeOverview ?? buildFallbackOverview(tokens, trackedSymbols);

  const isTracked = trackedSymbols.includes(resolvedContext.header.symbol);
  ...
  return (
    ...
    >
      {liveBackendUnavailable ? (
        <section className="mb-6 border border-amber-500/20 bg-amber-500/8 px-5 py-4 text-xs uppercase tracking-[0.24em] text-amber-200/88">
          Live backend data is temporarily unavailable. Scoutkat is showing a degraded workspace shell so you can continue setup.
        </section>
      ) : null}
          ...
          <Card className="border border-white/12 bg-white/[0.02] p-0">
            ...
            <div className="space-y-5 p-5">
              <form action="/app" className="space-y-3">
                ...
                <div className="flex gap-2">
                  <input
                    ...
                    name="symbol"
                    list="token-registry"
                    defaultValue={resolvedContext.header.symbol}
              ...
              <form action={toggleTrackedAsset}>
                <input type="hidden" name="symbol" value={resolvedContext.header.symbol} />
          ...
          <Card className="border border-white/12 bg-white/[0.02] p-0">
            ...
            <div className="grid">
              {tokenUniverse.visibleTokens.map((token, index) => {
                const active = token.symbol === resolvedContext.header.symbol;
        ...
        <div className="space-y-6">
          <Card className="border border-white/12 bg-white/[0.02] p-0">
            <div className="grid border-b border-white/12 lg:grid-cols-[minmax(0,1.15fr)_340px]">
              <div className="p-5 md:p-6">
                <div className="flex items-start justify-between gap-4">
                  <div>
                    ...
                    <h2 className="display mt-4 text-5xl text-white">{resolvedContext.header.symbol}</h2>
                    <p className="mt-3 text-xs uppercase tracking-[0.2em] text-white/42">{resolvedContext.header.name}</p>
                  </div>
                  <Badge variant="muted">{resolvedContext.header.market_type}</Badge>
                </div>
                ...
                <div className="mt-8 grid gap-3 md:grid-cols-4">
                  <div className="border border-white/12 px-4 py-4 md:col-span-2">
                    ...
                    <p className="mt-3 text-3xl text-white">{formatPrice(resolvedContext.header.current_price)}</p>
                  </div>
                  <div className="border border-white/12 px-4 py-4">
                    <p className="text-[10px] uppercase tracking-[0.24em] text-white/34">1h</p>
                    <p className={`mt-3 text-lg ${changeTone(resolvedContext.header.change_1h)}`}>
                      {formatPercent(resolvedContext.header.change_1h)}
                    </p>
                  </div>
                  <div className="border border-white/12 px-4 py-4">
                    <p className="text-[10px] uppercase tracking-[0.24em] text-white/34">24h</p>
                    <p className={`mt-3 text-lg ${changeTone(resolvedContext.header.change_24h)}`}>
                      {formatPercent(resolvedContext.header.change_24h)}
                    </p>
                  </div>
                </div>
              </div>
              ...
              <div className="border-t border-white/12 p-5 md:p-6 lg:border-l lg:border-t-0">
                ...
                <div className="mt-4">
                  <span className={`inline-flex px-2 py-1 text-[10px] uppercase tracking-[0.18em] ${signalTone(resolvedContext.current_signal_state.signal_type)}`}>
                    {signalLabel(resolvedContext.current_signal_state.signal_type)}
                  </span>
                </div>
                <p className="mt-5 text-4xl text-white">{Math.round(resolvedContext.current_signal_state.signal_score)}</p>
                <p className="mt-2 text-xs uppercase tracking-[0.2em] text-white/44">
                  Confidence {Math.round(resolvedContext.current_signal_state.confidence * 100)}%
                </p>
                <div className="mt-6 space-y-4 text-sm leading-7 text-white/62">
                  <div>
                    ...
                    <p>{resolvedContext.current_signal_state.why_now}</p>
                  </div>
                  <div>
                    <p className="mb-1 text-[10px] uppercase tracking-[0.24em] text-white/34">Risks</p>
                    <p>{resolvedContext.current_signal_state.risks.join(" · ")}</p>
                  </div>
                </div>
              </div>
            </div>
          ...
          <div className="grid gap-6 xl:grid-cols-[minmax(0,1.15fr)_360px]">
            <Card className="border border-white/12 bg-white/[0.02] p-6">
              <DivergenceChart
                defaultTimeframe={resolvedContext.divergence_chart.default_timeframe}
                series={resolvedContext.divergence_chart.series}
              />
            </Card>
            ...
            <Card className="border border-white/12 bg-white/[0.02]">
              ...
              <div className="mt-5 flex flex-wrap gap-2">
                <Badge variant={resolvedContext.social_summary.snapshot_incomplete ? "muted" : "accent"}>
                  {resolvedContext.social_summary.snapshot_incomplete ? "Partial snapshot" : "Live snapshot"}
                </Badge>
                <Badge variant="muted">{resolvedContext.social_summary.attention_label}</Badge>
              </div>
              <div className="mt-5 space-y-3">
                {summaryPoints(resolvedContext).map((point) => (
              ...
              <div className="mt-5 border-t border-white/12 pt-5 text-xs uppercase tracking-[0.2em] text-white/42">
                <p>{resolvedContext.social_summary.discussion_type.replaceAll("_", " ")}</p>
                <p className="mt-2">{resolvedContext.social_summary.signal_hint.replaceAll("_", " ")}</p>
                <p className="mt-2">Novelty {Math.round(resolvedContext.social_summary.narrative_novelty * 100)}%</p>
              </div>
            </Card>
          </div>
          ...
          <div className="grid gap-6 xl:grid-cols-[minmax(0,1fr)_360px]">
            <Card className="border border-white/12 bg-white/[0.02]">
              ...
              <div className="mt-5 space-y-4">
                {resolvedContext.recent_state_changes.map((item) => (
